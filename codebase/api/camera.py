...
class Camera:
    ...
    # Used to calculate bounding box overlap to suppress duplicates
    ...
    # Used to calculate IOU (Intersection over Union, intersection area of
    # bounding boxes) to filter out duplicate bounding boxes
    def calculate_iou(self, box, truth):
        ...
        width_overlap = self.calculate_overlap(
            box[0], box[2], truth[0], truth[2])
        height_overlap = self.calculate_overlap(
            box[1], box[3], truth[1], truth[3])
        # no overlap
    ...
    # Sorts and removes duplicate bounding boxes
    ...
    def post_processing(self, output, original_img):
        ...
        # The 125 results need to be re-organized into 5 chunks of 6 values
        # 1 class + 1 score + 4 coordinates = 6 values
        # 6 values for each of the 5 anchor bounding boxes = 30 values
        ...
        for row in range(num_grids):
            for col in range(num_grids):
                for anchor_box_num in range(num_anchor_boxes):
                    ...
                    box_x = (
                        col + self.sigmoid(reordered_results[row * 13 + col][anchor_box_num][0])) / 13.0
                    box_y = (
                        row + self.sigmoid(reordered_results[row * 13 + col][anchor_box_num][1])) / 13.0
                    ...
                    for class_enum in range(num_classes):
                        class_list.append(
                            reordered_results[row * 13 + col][anchor_box_num][5 + class_enum])

                    # perform a Softmax on the classes
                    highest_class_score = max(class_list)
                    for current_class in range(len(class_list)):
                        class_list[current_class] = np.exp(
                            class_list[current_class] - highest_class_score)

                    current_score_total = sum(class_list)
                    for current_class in range(len(class_list)):
                        class_list[current_class] = class_list[current_class] * \
                            1.0 / current_score_total

                    # probability that the current anchor box contains an item
                    object_confidence = self.sigmoid(
                        reordered_results[row * 13 + col][anchor_box_num][4])
                    # highest class score detected for the object in the current anchor box
                    highest_class_score = max(class_list)
                    # index of the class with the highest score
                    class_w_highest_score = class_list.index(
                        max(class_list)) + 1
        ...
        for box in _results:
            ...
            if avg < 0.15 or avg > 0.85:
                print("Result removed at avg x: " + str(avg)) if DEBUG else 0
                del results[j]
        ...
        for box in results:
            # essentially box y min divided by image height (percent from bottom of view)
        ...
        return (-1 if (len(results) == 0) else gymin)
    ...
    def run(self):
        ...
        self.graph.queue_inference_with_fifo_elem(
            self.fifo_in, self.fifo_out, resized_img, "user object")
        # get the result
        output, userobj = self.fifo_out.read_elem()

        # Tiny Yolo V2 requires post processing to filter out duplicate objects
        # and low score objects -- returns y coordinate of lowest bbox
